...
const SubmitButton = () => {
    ...
    const textLengthWithoutTags = state.comment
        .replace(/<[^>]*>/g, "")
        .trim().length;
...
export const CommentEditor = ({ postId }: { postId: string }) => {
    ...
    const handleAnimationState = (newState: keyof typeof fadeInUpVariants) => {
        if (state.isSubmitting || !state.isOpenCommentEditor) return;
        animation.start({ ...fadeInUpVariants[newState] });
    };

    const onHoverEnd = () => {
        if (isBlurred.current) {
            handleAnimationState("rest");
        }
    };
    ...
    useEffect(() => {
        animation.start(state.isOpenCommentEditor ? "visible" : "hidden");
    }, [state.isOpenCommentEditor, animation]);
        ...
        >
            <div className="relative">
                <motion.div
                    ...
                    onHoverStart={() => handleAnimationState("visible")}
